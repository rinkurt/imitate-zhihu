...
func GetQuestions(uid int64, search string, cursor []int64, size int, orderBy string) ([]dto.QuestionShortDto, result.Result) {
	//offset := (page - 1) * size
	questions, res := repository.SelectQuestions(uid, search, cursor, size, orderBy)
	if res.NotOK() {
		return nil, res
	}
